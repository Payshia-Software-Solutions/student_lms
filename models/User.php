...
class User
{
    ...
    {
        $stmt = $this->pdo->prepare("SELECT id, f_name, l_name, email, nic, user_status, student_number, company_id, is_active, created_at, updated_at FROM users WHERE is_active = 1");
        $stmt->execute();
    ...
    {
        $stmt = $this->pdo->prepare("SELECT id, f_name, l_name, email, nic, user_status, student_number, company_id, is_active, created_at, updated_at FROM users WHERE id = ? AND is_active = 1");
        $stmt->execute([$id]);
