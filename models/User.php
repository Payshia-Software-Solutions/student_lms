...
class User
{
    ...
    {
        $stmt = $this->pdo->prepare("SELECT id, f_name, l_name, email, nic, user_status, is_active, created_at, updated_at FROM users WHERE is_active = 1");
        $stmt->execute();
    ...
    {
        $stmt = $this->pdo->prepare("SELECT id, f_name, l_name, email, nic, user_status, is_active, created_at, updated_at FROM users WHERE id = ? AND is_active = 1");
        $stmt->execute([$id]);
    ...
    {
        $stmt = $this->pdo->prepare("\n            INSERT INTO users (f_name, l_name, email, password, nic, user_status, created_by)\n            VALUES (:f_name, :l_name, :email, :password, :nic, :user_status, :created_by)\n        ");
